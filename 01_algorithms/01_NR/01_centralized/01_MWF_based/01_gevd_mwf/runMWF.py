...
CWD = os.getcwd()
PLOT_RESULTS =   0  # If true, plots speech enhancement results on figures (+ exports if SAVEFIGS) 
...
def main():
    ...
    noise_spatially_white = 0       # If true, assume spatially white noise and ignore specific noise source positions
    ...
    for idxAS in range(nAS):
        ...
        # Re-shape <W_hat>
        W_hat = np.transpose(W_hat, [2,0,1])
            ...
            for ii, band_indices in enumerate(bands_indices):
                if len(band_indices) > 0:
                    ...
                    myMWF.spatial_visu_MWF(W_hat_curr, freqs_curr, rd, alpha, r, Fs, rir_dur, targetSources=rs, noiseSources=rn,\
                        exportit=1, exportname=fname, noise_spatially_white=noise_spatially_white)
                else:
