"""ABOUT
Dynamic Weighted Average Coherence Drift (DWACD) for SRO estimation.
Custom implementation, based on original implementation (see references
below)
...
GitHub: https://github.com/fgnt/paderwasn
"""
...
class DWACDParameters():
    """Handy class containing all revelant input DWACD parameters.
    -- Also contains handy functions.
    Attributes:
        seg_len (int):
    ...
    seg_len : int = 8192
    seg_shift : int = 2048
    frame_shift_welch : int = 512
    fft_size : int = 4096
    temp_dist : int = 8192
    alpha : float = .95
    src_activity_th : float = .75
    settling_time : float = 40

    def __post_init__(self):
        self.stft = STFT(shift=self.frame_shift_welch, size=self.fft_size,
                            window_length=self.fft_size, pad=False, fading=False)
        self.src_activity_th = self.src_activity_th * self.seg_len
...
def dwacd_chunk(seg_idx, sro_est, tau_sro, self: DWACDParameters, sig, ref_sig, activity_sig, activity_ref_sig):
    """Chunk-wise processing of DWACD."""
    ...
    activity_seg = activity_sig[start+shift:start+shift+self.seg_len]
    ...
    activity_seg_ref_delayed = activity_ref_sig[start_delayed:start_delayed+self.seg_len]
    activity = (
    ...
    if activity:
        ...
        coherence_delayed = self.calc_coherence(seg_delayed, seg_ref_delayed, sro_est)
        ...
        avg_coh_prod = (self.alpha * avg_coh_prod + (1 - self.alpha) * coherence_product)
    ...
    return sro_est, tau_sro, nxt_end


def dwacd(self: DWACDParameters, sig, ref_sig, activity_sig, activity_ref_sig):
    ...
    for seg_idx in range(num_segments):
        ...
        sro_est, tau_sro, nxt_end = dwacd_chunk(seg_idx, sro_est, tau_sro, self, sig, ref_sig, activity_sig, activity_ref_sig)
